...
-- uartRx :: Signal Bit -> Signal Bool -> (Signal Byte, Signal Bool)
...
uartRx :: Signal Bit -> (Signal Byte, Signal Bool)
uartRx rx = (rx_dout, rx_done_tick)
  where
  (rx_dout, rx_done_tick) = mealyB rxRun rxInit (rx, baud_tick)

uartTx :: Signal Bool -> Signal Byte -> (Signal Bit, Signal Bool)
uartTx tx_write tx_data = (tx, fifo_full)
  where
  (tx, tx_done_tick) = mealyB txRun txInit (not <$> fifo_empty, fifo_r_data, baud_tick)
